...
def get_scheduler(optim, total_epochs, last_epoch):

    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optim, T_max=total_epochs, eta_min=1e-5, last_epoch=last_epoch
    )
    return scheduler
...
def main():
    ...
    scheduler = get_scheduler(optimizer, total_epochs=args.epochs, last_epoch=last_epoch)

    net = net.train()
    logger.info('**********************Start training**********************')

    train_model(net, optimizer, train_loader, scheduler, it, start_epoch, args.epochs, ckpt_dir, logger=logger)
