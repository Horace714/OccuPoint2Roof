...
def train_one_epoch(model, optim, data_loader, accumulated_iter,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        for k, v in disp_dict.items():
            ...
            epoch_metrics_sum[k] = epoch_metrics_sum.get(k, 0.0) + val
    ...
    avg_metrics = {}
    if valid_batches > 0:
        for key, val_sum in epoch_metrics_sum.items():
            avg_metrics[key] = val_sum / valid_batches

    return accumulated_iter, avg_metrics

def train_model(model, optim, data_loader, lr_sch, start_it, start_epoch, total_epochs, ckpt_save_dir, logger=None,
                sampler=None, max_ckpt_save_num=5):
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True) as tbar:
        ...
        for e in tbar:
            ...
            accumulated_iter, avg_metrics = train_one_epoch(model, optim, data_loader, accumulated_iter, tbar,
                                                            leave_pbar=(e + 1 == total_epochs))

            if logger is not None:
                metrics_str = ", ".join(
                    [f"{k}={v:.3f}" if k != 'lr' else f"{k}={v:.6f}" for k, v in avg_metrics.items()])

                log_msg = f'Epoch [{e}/{total_epochs}] | {metrics_str}\n'

                for handler in logger.handlers:
                    if hasattr(handler, 'baseFilename'):
                        handler.stream.write(log_msg)
