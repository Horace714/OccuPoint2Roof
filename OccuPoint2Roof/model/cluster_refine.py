...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, output, targets):
        ...
        for i in range(batch_size):
            ...
            dist_matrix = torch.cdist(tmp_output, tmp_targets)

            valid_outputs.append(tmp_output)
            valid_targets.append(tmp_targets)
            dist_mats.append(dist_matrix)
            new_xyz_batch_cnt.append(tmp_output.shape[0])

        dist_mats_np = [d.cpu().numpy() if d is not None else None for d in dist_mats]

        for i in range(batch_size):
            if dist_mats_np[i] is None:
                continue

            tmp_output = valid_outputs[i]
            tmp_targets = valid_targets[i]
            dist_matrix = dist_mats[i]
            dist_matrix_np = dist_mats_np[i]

            out_ind, tar_ind = linear_sum_assignment(dist_matrix_np)

            out_ind = dist_matrix.new_tensor(out_ind, dtype=torch.int64)
            tar_ind = dist_matrix.new_tensor(tar_ind, dtype=torch.int64)

            dist_val = dist_matrix[out_ind, tar_ind]
            valid_mask = dist_val < self.dist_thresh

            out_ind = out_ind[valid_mask]
            tar_ind = tar_ind[valid_mask]
        ...
        return torch.cat(pts_list, 0), torch.cat(target_list, 0), torch.cat(label_list, 0), torch.cat(match_list,
                                                                                                      0), output.new_tensor(
            new_xyz_batch_cnt, dtype=torch.int32)
