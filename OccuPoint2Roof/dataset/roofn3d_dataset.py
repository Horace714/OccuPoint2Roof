...
class RoofN3dDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        points = read_pts(file_path + '/points_occluded.xyz')
        ...
        vectors, edges = load_obj(file_path + '/polygon.obj')

        mask_path = file_path + '/visible_mask.npy'
        if os.path.exists(mask_path):
            visible_mask = np.load(mask_path).astype(bool)
        else:
            visible_mask = np.ones(len(vectors), dtype=bool)
        ...
        visible_corners_gt = vectors[visible_mask]
        occluded_corners_gt = vectors[~visible_mask]
        ...
        data_dict = {
            ...
            'frame_id': frame_id,
            'minMaxPt': pt,
            'visible_corners_gt': visible_corners_gt,
            'occluded_corners_gt': occluded_corners_gt
        }
        return data_dict
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['vectors', 'edges', 'visible_corners_gt', 'occluded_corners_gt']:
                    max_vec = max([len(x) for x in val])
                    if max_vec == 0: max_vec = 1
                ...
                print(f'Error in collate_batch: key={key}, Exception: {e}')
                raise TypeError
